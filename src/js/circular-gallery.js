...
class Media {
  constructor({ geometry, gl, image, index, length, renderer, scene, screen, text, viewport, bend, textColor, borderRadius = 0, font, onClick }) {
    this.extra = 0;
  ...
  createMesh() {
    ...
    if (this.onClick) {
      this.plane.userData = { href: this.onClick };
    }
  }
  createTitle() {
...
export function initCircularGallery(container, { items, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 24px Inter', scrollSpeed = 1.5, scrollEase = 0.05 } = {}) {
  let app;
